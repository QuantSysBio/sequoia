...
#               - (optional) quantification of transcriptome
...
min_ORF_length = 8
alt_init_codons = "ATG|CTG|GTG|ATC|ACG"
# attributes(GENETIC_CODE)$alt_init_codons <- c("TTG", "CTG") # original
attributes(GENETIC_CODE)$alt_init_codons <- c("CTG","GTG","ATC","ACG") # cryptic

# GTF annotation
input <- list()
input$gtf_all <- read_gff(file = paste0("data/gencode.v33.annotation.gtf.gz"),
                          genome_info = "hg38")
input$gtf_all
...
for (stratum in genomic_strata) {
  print(stratum)
  for (chromosome in seqlevels_txdb) {
    ...
    if (length(stratum_coords) > 0) {
      ...
      fasta_files$ORFs_noCDS %>%
        ...
        unique() %>%
        writeXStringSet(filepath = paste0("results/all/", stratum, "_ORFs_noCDS.fa"), append = T)
      
      fasta_files$ORFs_CDS_frameshift %>%
        translate(no.init.codon = T, if.fuzzy.codon = "solve") %>%
        unique() %>%
        writeXStringSet(filepath = paste0("results/all/", stratum, "_ORFs_frameshift.fa"), append = T)
      
      fasta_files$ORFs_CDS_inframe %>%
        translate(no.init.codon = T, if.fuzzy.codon = "solve") %>%
        unique() %>%
        writeXStringSet(filepath = paste0("results/all/", stratum, "_ORFs_inframe.fa"), append = T)
      ...
      annotation_tables$ORFs_noCDS %>%
        select(-ORF_name) %>%
        unique() %>%
        vroom_write(file = paste0("results/all/", stratum, "_ORFs_noCDS.csv"), delim = ",", append = T, num_threads = Ncpu)
      
      annotation_tables$ORFs_CDS_frameshift %>%
        select(-ORF_name) %>%
        unique() %>%
        vroom_write(file = paste0("results/all/", stratum, "_ORFs_frameshift.csv"), delim = ",", append = T, num_threads = Ncpu)
      
      annotation_tables$ORFs_CDS_inframe %>%
        select(-ORF_name) %>%
        unique() %>%
        vroom_write(file = paste0("results/all/", stratum, "_ORFs_inframe.csv"), delim = ",", append = T, num_threads = Ncpu)
...
dir_res <- c("results/all/", "results/expressed/")
for (dir in dir_res) {
